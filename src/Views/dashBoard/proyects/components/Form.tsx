...
import { ProyectsContext } from '../../../../context/ProyectsContext';
  const uploadSchema = Yup.object().shape({
    ...
    description: Yup.string().required("La descripcion del proyecto es requerida").min(10),
  });
const FormProyects = ({closeForm}:any) => {
    ...
    return (<>
      ...
      validationSchema={uploadSchema}
            onSubmit={(values) => {
          setLoader(true)
          ApiController.AddWorkspaces({ value: values, img: images ,id:selectedCompany._id}).then(e => {
              console.log(e)
              closeForm()
              setUpdate(true)
              setLoader(false)
