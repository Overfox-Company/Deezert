...
import { PRIMARY_COLOR,CONTAINER,CONTAINER_DARK} from "../../constants/Color";
...
export const MenuMobile = ({ close }: any) => {
  const { darkMode, user, panel, setPanel,selectedCompany } = React.useContext(AppContext);
  const handleClick = (index: number) => {
    setPanel(index);
  };
  const data = selectedCompany?.idOwner ===user._id?AdmFunctions:UserFunctions
  return (
    <>
      <Box
...
export const Menu = () => {
  const { darkMode, user, panel, setPanel,selectedCompany } = React.useContext(AppContext);
    ...
    const data = selectedCompany?.idOwner === user._id?AdmFunctions:UserFunctions
  return (
    <FixedContainer style={{backgroundColor: darkMode?CONTAINER_DARK:CONTAINER}}>
      <Grid container>
